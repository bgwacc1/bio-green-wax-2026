...
*   **SEO Optimization:**
    ...
    *   **Bot Pre-rendering Middleware (`frontend/plugins/bot-prerender.ts`):** Vite plugin that intercepts requests from search engine bots and SEO tools (Googlebot, Bingbot, Semrush, Screaming Frog, etc.) and serves fully rendered HTML with proper H1 tags, structured data (JSON-LD), meta tags, hreflang alternates, navigation, and page content. Fetches data from the PHP API and generates SEO-optimized HTML for all page types: Home, Products (list + detail), About, News (list + detail), Contact, Certifications, Careers, and Sectors. Results cached in-memory for 1 hour. Normal browser users continue to receive the React SPA.
    *   **Static fallback with JSON-LD (`frontend/plugins/static-fallback.ts`):** Injects full structured HTML into `<!--STATIC_FALLBACK-->` placeholder in index.html for non-JS environments. Includes `@graph`-wrapped JSON-LD (Organization, WebSite, BreadcrumbList, 10 Products, Articles) in all 16 languages. Auto-refreshes every 1 hour.
    *   **Critical CSS inlining (`frontend/plugins/critical-css.ts`):** Vite plugin that inlines ~2KB of essential above-the-fold CSS (CSS variables, box model reset, typography, body/header/main layout, container-wide, section-padding) directly into the HTML `<head>` as a `<style>` block. Converts Vite's render-blocking CSS `<link rel="stylesheet">` to async loading via `<link rel="preload" as="style" onload="this.rel='stylesheet'">` with `<noscript>` fallback. Eliminates the ~680ms render-blocking CSS delay flagged by Lighthouse.
    *   **Non-blocking font loading:** Google Fonts loaded via `<link>` with `media="print" onload="this.media='all'"` pattern + `preconnect` hints (not render-blocking `@import`).
    *   **`<meta name="robots" content="index, follow">`** explicitly set in `index.html`.
*   **Website Analytics System:** A self-hosted, privacy-compliant analytics solution tracks page views, geographic location, device info, session duration, referrer source, and more. Data is stored in `visitor_sessions` and `page_views` tables, with an admin dashboard providing interactive charts for analysis. Data older than 3 months is automatically purged.
    *   **AI Bot Analytics:** Dedicated bot visit tracking via `bot_visits` table logs every AI crawler visit with bot name, user agent, IP, geolocation, page path, page type, language, and domain. Bot visits are logged in both environments: Vite dev server (`bot-prerender.ts` → `POST /analytics/bots/log`) and Ionos production (`bot-renderer.php` → direct DB insert). Both handle column name differences between PostgreSQL (`user_agent`) and MySQL (`bot_user_agent`) automatically. Admin dashboard "AI Bots" sub-tab under Analytics shows summary stats, visits over time (line chart), visits by bot (pie chart), by language (bar chart), most crawled pages (table), by country (progress bars), and recent visit log.
