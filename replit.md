...
*   **SEO Optimization:**
    ...
    *   **Static fallback with JSON-LD (`frontend/plugins/static-fallback.ts`):** Injects full structured HTML into `<!--STATIC_FALLBACK-->` placeholder in index.html for non-JS environments. Includes `@graph`-wrapped JSON-LD (Organization, WebSite, BreadcrumbList, 10 Products, Articles) in all 16 languages. Auto-refreshes every 5 minutes.
